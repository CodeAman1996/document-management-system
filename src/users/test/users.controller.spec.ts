...
describe('UsersController', () => {
  ...
  describe('register (public)', () => {
    it('should create a user without role protection', async () => {
      const dto: RegisterDto = {
        name: 'Aman', email: 'aman@mail.com', password: '123',
        place: ''
      };
  ...
  describe('create (admin only)', () => {
    it('should create a user when called by admin', async () => {
      const dto: RegisterDto = {
        name: 'AdminUser', email: 'admin@mail.com', password: 'secure',
        place: ''
      };
    ...
    it('should throw if user creation fails', async () => {
      ...
      await expect(controller.create({} as any)).rejects.toThrow('Failed to create user');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw error on failure', async () => {
      ...
      await expect(controller.update('1', {} as any)).rejects.toThrow('Failed to update user');
    });
  });
  ...
  describe('remove', () => {
    ...
    it('should throw error if deletion fails', async () => {
      ...
      await expect(controller.remove('1')).rejects.toThrow('Failed to delete user');
    });
  });
});
